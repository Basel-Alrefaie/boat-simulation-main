...
let physic = new Physics(5.23, 2.09, 0.95, 450, 0.1, 0.1, 500);
...
gui.add(params, 'rpm', 0, 7000).name('RPM').onChange(updateBoatParams);
...
gui.add(params, 'airVelocity').name('Air Velocity').onChange(updateBoatParams);
gui.add(params, 'waterVelocity').name('water Velocity').onChange(updateBoatParams);
...
function updateBoatParams() {
  if (boatModel) {
    ...
    physic.updateParams(params.boatLength, params.boatWidth, params.boatHeight, params.boatMass,params.airVelocity,params.waterVelocity,params.rpm);
  }
}
...
function animate() {
  ...
  if (boatModel) {
    if (x < 10) {
//    console.log(boatModel.position, {elapsedtime, oldElapsedTime, deltatime, time });
      x += 1;
    }
...
//      boatModel.position.z += deltatime * 100;
...
document.addEventListener("keydown", (event) => {
 ...
 //console.log("boat mode position"+boatModel.x)
//console.log(physic.position, boatModel.position);
    /*
      model.scale.set(modelScale.x, modelScale.y, modelScale.z);
      model.position.set(modelLocation.x, modelLocation.y, modelLocation.z);
      model.rotation.set(modelRotations.x, modelRotations.y, modelRotations.z);
